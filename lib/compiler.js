...
function segmentIterator(segments, mods) {
  let index = 0;
  ...
  return {
    ...
    consume: function (cb) {
      const modifiers = mods.concat(this.current.modifiers || []);
      ...
      return segmentIterator(segments.slice(startIndex, index), modifiers);
    }
  };
}
...
function buildOutputSegment(iterator, options) {
  ...
  while (iterator.current && (iterator.current.type === 'text' || iterator.current.type === 'output')) {
    ...
    if (iterator.current.type === 'text') {
      ...
      if (iterator.current.expression.length) {
