...
var debug = false;
var ignoreSuspiciousSegments = false;
var beautify;
...
Object.defineProperties(Compiler, {
 'BEAUTIFY': {
  ...
  'IGNORE_SUSPICIOUS_SEGMENTS': {
    ...
    enumerable: true,
    get: function () {
      return ignoreSuspiciousSegments;
    },
    set: function (val) {
      ignoreSuspiciousSegments = val;
    }
  }
});
...
function compile(segments) {
  ...
  try {
    ...
    return vm.runInNewContext('f=' + template, { console: console });
  } catch (e) {
    if (debug) {
      throw e;
    } else {
