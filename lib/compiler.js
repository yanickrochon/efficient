...
const TEMPLATES = {
  ...
  // Segment: iterator
  'seg.iterator': 'engine.iterator(__CTX__,__EXPR__,_=FN=_)',

  // Segment: named
  'seg.named.declare': 'engine.setSegment(__EXPR__,_=FN=_)',
  'seg.named.render': 'engine.getSegment(__EXPR__)(__CTX__)',

  // Segment: custom
  'seg.custom': 'engine.callCustom(__CTX__,__PATH__,__SEGLIST__,__MODIFIER__)',
...
function buildOutputSegment(iterator, options) {
  ...
  while (iterator.current && (iterator.current.type === 'text' || iterator.current.type === 'output')) {
    ...
    if (iterator.current.type === 'text') {
      ...
      output.push(wrapModifiers(iterator.current.modifiers, buildExpression(iterator.current.expression)));
      previousText = false;
    }
...
function buildSegmentConditional(iterator, options, count) {
  ...
  code = (count === 1 && getDebugCode(segment, options, ';') || '') + getTemplate(templateName, {
    expr: segment.expression && buildExpression(segment.expression),
    code: chunk.code
  });
...
function buildSegmentIterator(iterator, options, count) {
  ...
  code = 'return ' + getDebugCode(segment, options, ',') + getTemplate('seg.iterator', {
    'expr': buildExpression(segment.expression),
    'code': chunk.code
  });
...
function buildSegmentCustom(iterator, options) {
  var segment = iterator.current;
  var expression = buildExpression(iterator.current.expression);
  var segments = [];
  var code;
  var subIterator;
  var chunk;
  ...
  while (!iterator.current.closing) {
    ...
    if (!chunk.async) {
      chunk.code = getTemplate('promise', {
        ctx: ''
      }) + getTemplate('promise.then', {
        ctx: '',
        code: chunk.code
  ...
  code = 'return ' + getDebugCode(segment, options, ',') + getTemplate('seg.custom', {
    path: expression,
    seglist: '[' + segments.join(',') + ']',
    modifier: getTemplate('fn', {
      ctx: 'str',
...
function buildSegmentNamedDeclare(iterator, options, count) {
  ...
  code = getDebugCode(segment, options, ';') + getTemplate('seg.named.declare', {
    'expr': buildExpression(segment.expression),
    'code': chunk.code
  });

  return {
    async: false,   // declaring is sync.... rendering is assumed always async
    code: code
  };
}
...
function buildSegmentNamedRender(iterator, options, count) {
  ...
  code = 'return ' + getDebugCode(segment, options, ',') + getTemplate('seg.named.render', {
    'expr': buildExpression(segment.expression)
  }) + ';';

  return {
    async: true,   // declaring is sync.... rendering is assumed always async
    code: code
  };
}
...
function buildSegmentPartial(iterator, options, count) {
  ...
  code = 'return ' + getDebugCode(segment, options, ',') + getTemplate('seg.partial', {
    'expr': buildExpression(segment.expression)
  });

  return {
    async: true,   // this invoke engine.render, which is async
    code: code
  };
}
