...
var InternalEngine = function InternalEngine(engine, data) {
  ...
  Object.defineProperties(this, {
    ...
    'callCustom': {
      ...
      configurable: false,
      writable: false,
      value: function callCustomFunction(ctx, path, segments, outputModifier) {
        var engine = this;
...
function resolveTemplate(name, engine) {
  ...
  if (Engine._cache[key]) {
    ...
    return Engine._cache[key] = new Promise(function (resolve, reject) {
      ...
      var pathIndex = 0;
      var pathPrefix = Object.keys(paths).filter(function (prefix) {
        return (typeof prefix === 'string') && ((prefix === '*') || (key.substr(0, prefix.length) === prefix));
      });
      var pathCount;
      ...
      (function nextFile() {
        ...
        if (pathIndex >= pathCount) {
          ...
          file = path.join(paths[prefix], (prefix === '*' ? name : name.substr(prefix.length)) + ext[extIndex]);
...
function renderTemplate(name, ctx, internalEngine, engine) {
  ...
  Object.defineProperty(promise, 'stream', {
    enumerable: true,
    configurable: false,
    writable: false,
