...
const MODIFIERS = {
  ...
  'lower': modifierLower,
  'mask': modifierMask,
  'lpad': modifierLeftPad,
  'rpad': modifierRightPad
};
...
function registerModifier(modifier) {
  ...
  } else if (MODIFIERS[modifier.name]) {
...
Modifier
*/
function modifierLeftPad(val, size, c) {
  return pad(size || 1, String(val), c || ' ');
}

/**
Modifier
*/
function modifierRightPad(val, size, c) {
  return pad(String(val), size || 1, c || ' ');
}
