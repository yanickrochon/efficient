...
const TEMPLATES = {
  'template': 'function(engine,ctx){var c=ctx,ptr;return __CODE__}',
  'template.debug': 'ptr={column:__COLUMN__,line:__LINE__,pos:__POS__}',

  'promise': 'Promise.resolve(__CTX__).then(__FN__).catch(function(err){engine.err(err,ptr);}).then(function(){return c;})',
  'promise.then': ').then(',
...
function getDebugCode(segment) {
  if (debug) {
    return getTemplate('template.debug', {
      'column': segment.column || NaN,
      'line': segment.line || NaN,
      'pos': segment.pos || NaN
    }) + ';';
  }
  return '';
...
function buildOutputSegment(iterator) {
  ...
  while (iterator.current && (iterator.current.type === 'text' || iterator.current.type === 'output')) {
    ...
    if (iterator.current.type === 'text') {
      ...
      output.push('String(' + buildExpression(iterator.current) + ')');
      previousText = false;
    }
...
function buildSegment(iterator) {
  ...
  var segments = [];
  var subIterator;
  var code = getDebugCode(iterator.current);
  var buildResult;
  var templateOptions;
...
function buildSegmentCustom(testExpr, segments) {
  return {
    requiresReturn: true,
...
function buildSegmentNamedDeclare(testExpr, segments) {
  var seg;
  var templateOptions;

  if (segments.length > 1) {
    throw CompilerException('Too many segments for named segment');
  }
...
function buildSegmentNamedRender(testExpr, segments) {
  if (segments.length) {
    throw CompilerException('Too many segments for named segment');
  }
...
function buildSegmentPartial(testExpr, segments) {
  if (segments.length) {
    throw CompilerException('Too many segments for partial');
  }
  ...
  return (function _expr(content) {
    ...
    for (var i = 0, len = content.length; i < len; ++i) {
      switch (content[i].type) {
        ...
        case 'number':
        case 'context':
          if (content[i].arguments) {
            right = '(' + _expr(content[i].arguments) + ')';
          } else {
            right = '';
          }

          valueStack.push(content[i].value + right);
          break;
        case 'string':
