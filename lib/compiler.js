...
function compile(segments, options) {
  ...
  try {
    ...
    if (!chunk.async) {
      chunk.code = getTemplate('promise') + (chunk.code && (getTemplate('promise.then', {
        code: chunk.code
      })));
    }
...
/*
    ...
    while (iterator.current && iterator.current.type === segType) {
      if (iterator.current.closing) {
        ...
        segments.push({
          //expression: 
          code: build(subIterator, options)
        });
    ...
    if (buildResult.code) {
      if (initialSegment.context && !buildResult.contextHandled) {
        ...
        buildResult.code = getTemplate('ctx.set', {
          'path': quote(initialSegment.context),
...
function buildSegmentIterator(iterator, options, count) {
  ...
  if (segment.context) {
    code = getTemplate('promise', {
      ctx: getTemplate('ctx.get', {
        path: quote(segment.context)
      })
