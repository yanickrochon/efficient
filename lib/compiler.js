...
const TEMPLATES = {
  'template': 'function(__ENGINE__,__CTX__){_=TEMPLATE.DEBUG.INIT=_;return __TEMPLATE__;}',
...
function build(iterator, options) {
  ...
  while (iterator.current) {
    ...
    if (builder) {
      chunk = builder(iterator, options);
...
function buildSegmentConditional(iterator, options, count) {
  ...
  var debug = getDebugCode(segment, options);
  var async = false;
  var code;
