...
const TEMPLATES = {
  ...
  'ctx.get': 'ctx.getContext(__PATH__)',
  ...
  // Segment: iterator
  'seg.iterator': 'engine.iterator(__EXPR__,__CTX__,_=FN=_)_=PROMISE.RETURN=_',

  // Segment: named
  'seg.named.declare': 'engine.setSegment(__EXPR__,__CTX__,__FN__)',
  'seg.named.render': 'engine.getSegment(__EXPR__)(__CTX__)_=PROMISE.RETURN=_',

  // Segment: custom
  'seg.custom': 'engine.callCustom(__EXPR__,__CTX__,__SEGMENTS__)_=PROMISE.RETURN=_',

  // Segment: partial
  'seg.partial': 'engine.render(__EXPR__,__CTX__)_=PROMISE.RETURN=_'
...
function getTemplate(name, tokens) {
  ...
  return TEMPLATES[name].replace(TEMPLATE_TPL, function (match, name) {
    ...
    return token in tokens ? String(tokens[token]) : (debug ? ('/*?' + token + '*/') : '');
  });
}
      ...
      if (context !== undefined && context !== iterator.current.content.context && output.length) {
        ...
        if (context) {
          code += getTemplate('ctx.set', {
            path: quote(context),
            code: getDebugCode(contextSeg) + getTemplate('out', {
              'value': output.join('+')
            }) + ';'
          }) + ';';
        } else {
    ...
    if (buildResult.code) {
      if (initialSegment.content.context && !buildResult.contextHandled) {
        buildResult.code = getTemplate('ctx.set', {
          ...
          'code': buildResult.code && (getDebugCode(initialSegment) + buildResult.code) || ''
        });
      } else {
        buildResult.code = getDebugCode(initialSegment) + buildResult.code;
      }

      if (buildResult.async) {
