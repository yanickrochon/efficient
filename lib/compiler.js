...
function buildSegmentConditional(iterator, options) {
  ...
  /*
    ...
    // else if (async)
      ...
      code = code + getTemplate('seg.conditional.else', {
        code: wrapCodeSegment(chunk.async, debug, getTemplate('seg.conditional', {
    ...
    // else if (sync)
      ...
      code = code + getTemplate('seg.conditional.elseif', {
        expr: segment.expression && buildExpression(segment.expression, initBuffer),
        code: chunk.code
      });
    // else
      ...
      code = code + getTemplate('seg.conditional.else', {
        code: wrapCodeSegment(chunk.async, debug, chunk.code, initBuffer).code
      });
    }
  */
...
function buildSegmentCustom(iterator, options) {
  const segment = iterator.current;
  const initBuffer = [];
  let expression = buildExpression(iterator.current.expression, initBuffer);
  let segments = [];
  let debug;
  let code;
  let subIterator;
...
function buildSegmentNamedDeclare(iterator, options, count) {
  ...
  if (iterator.current && iterator.current.type === 'iterator' && iterator.current.next) {
