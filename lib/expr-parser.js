...
const RULES = [
  ...
  { pattern: /true|false|null|undefined|NaN/,                                     action: lexRuleReserved },
  { pattern: /((?:~|\.+)?(?:[a-zA-Z_]\w*\.)*[a-zA-Z_]\w*)(?:\:((?:[a-zA-Z_]\w*(?:\[\d+\])?\.)*[a-zA-Z_]\w*(?:\[\d+\])?))?/, action: lexRuleContext },
  { pattern: /[-+]?(?:\d*\.\d+|\d+)/,                                             action: lexRuleNumber },
  { pattern: /"((?:\\\"|[^\r\n]+)+)"/,                                            action: lexRuleString },
...
const STATES = {
  ...
  'parenOpen':  [ 'negate', 'reserved', 'number', 'string', 'context', 'parenOpen', 'parenClose' ],
...
function lexRuleAccept(type) {
  return function lexRuleAcceptToken(ctx, match) {
    ctx.accept(type);
  };
}

function lexRuleReserved(ctx, match) {
  var value;
...
function lexRuleContext(ctx, match) {
  ctx.accept('context', 'ctx.getContext("' + match[1] + '").data' + (match[2] ? '.' + match[2] : ''));
}

function lexRuleString(ctx, match) {
...
ExpressionParser.prototype.parse = function parseExpression(expr) {
  ...
  if (parseStatement(this._lexer, tokenStack) !== 'EOF') {
    ...
    if (e instanceof ParserException) {
      ...
      throw ParserException('Syntax error on line ' + this._lexer._line + ', column ' + this._lexer._column);
    }
  }
...
function parseContextArguments(lexer, contextToken) {
  ...
  if (token && token.type === 'parenOpen') {
    ...
    contextToken.arguments = tokenStack;
