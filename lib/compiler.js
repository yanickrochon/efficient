...
const TEMPLATES = {
  ...
  'fn': 'function(c,ctx){__CODE__return c;}',
  ...
  while (iterator.current && (iterator.current.type === 'text' || iterator.current.type === 'output')) {
    ...
    if (iterator.current.type === 'text') {
      ...
      if (iterator.current.value.context) {
        if (context !== iterator.current.value.context && output.length) {
...
function buildSegmentConditional(iterator, compiled) {
  ...
  while (iterator.current && iterator.current.type === 'segment' && iterator.current.value.type === 'conditional') {
